...
int FuzzerDriver(int *argc, char ***argv, UserCallback Callback) {
  ...
  Options.OnlyPrintableASCII = Flags.only_printable_ascii;
  Options.OnlyDict = Flags.only_dict;
  Options.OutputCSV = Flags.output_csv;
  Options.DetectLeaks = Flags.detect_leaks;
  Options.TraceMalloc = Flags.trace_malloc;
  Options.RssLimitMb = Flags.rss_limit_mb;
  Options.MutationStart = Flags.mutation_start;
  Options.ScoreBuckets = Flags.score_buckets;
  Options.DeathMode = Flags.death_mode;
  if (Options.MutationStart == 0)
    Options.MutationStart = -1;
  Options.MaxMutationLen = Flags.max_mut_len;
  if (Flags.runs >= 0)
  ...
  unsigned char DictChar = '\n';
  for (auto &U: Dictionary)
    if (U.size() <= Word::GetMaxSize()) {
      // If a single character, also add to the character dictionary
      if (U.size() == 1) {
        MD->AddWordToManualChrDictionary(Word(U.data(), U.size()));
        DictChar = U.data()[0];
      }
  ...
  if (InitialCorpus.empty()) {
    InitialCorpus.push_back(Unit({DictChar}));  // Valid ASCII input.
