...
bool Fuzzer::RecordMaxCoverage(Fuzzer::Coverage *C) {
  ...
  if (Options.UseCounters) {
    uint64_t NumCounters = EF->__sanitizer_get_number_of_counters();
    uint64_t CounterDelta =
        EF->__sanitizer_update_counter_bitset_and_clear_counters(
            C->CounterBitmap.data());
    uint64_t num_new_bits = 0;
    for (uint32_t i = 0; i < NumCounters; i++) {
      if (!C->CounterBitmap.data()[i])
        continue;
      if ((C->CounterBitmap.data()[i] & C->CounterBitmap.data()[i]) ^ C->CounterBitmap.data()[i]) {
            C->CounterBitmap.data()[i] |= C->CounterBitmap.data()[i];
      }
      // Update raw edge counts using the upper limit of the bucket, with the
        // exception of the biggest bucket.
        if (C->CounterBitmap.data()[i] & 0x01) raw_edge_count += 1;   // 1
        else if (C->CounterBitmap.data()[i] & 0x02) raw_edge_count += 2;   // 2
        else if (C->CounterBitmap.data()[i] & 0x04) raw_edge_count += 3;   // 3
        else if (C->CounterBitmap.data()[i] & 0x08) raw_edge_count += 7;   // 4-7
        else if (C->CounterBitmap.data()[i] & 0x10) raw_edge_count += 15;  // 8-15
        else if (C->CounterBitmap.data()[i] & 0x20) raw_edge_count += 31;  // 16-31
        else if (C->CounterBitmap.data()[i] & 0x40) raw_edge_count += 127; // 32-127
        else if (C->CounterBitmap.data()[i] & 0x80) raw_edge_count += 255; // 128+
      }

    if (raw_edge_count > MaxEdges) {
      Res = true;
      MaxEdges = raw_edge_count;
      C->CounterBitmapBits += CounterDelta;
    }
  }

  EdgeNumber = raw_edge_count;
  return Res;
}
...
Fuzzer::Fuzzer(UserCallback CB, InputCorpus &Corpus, MutationDispatcher &MD,
  ...
  // if (Options.MaxMutationLen)
    // MaxMutationLen = Options.MaxMutationLen;
...
void Fuzzer::PrintStats(const char *Where, const char *End, size_t Units) {
  ...
  if (auto TB = MaxCoverage.CounterBitmapBits)
    Printf(" bits: %zd", TB);
  Printf(" raw_edges: %zd", MaxCoverage.TotalEdges);
...
size_t Fuzzer::RunOne(const uint8_t *Data, size_t Size) {
  ...
  // Check if instruction-based fitness function is enabled
  uint32_t edges = TPC.GetEdges();
  // if (edges > MaxCoverage.TotalEdges) {
  if (edges > MaxCoverage.TotalEdges) {
    auto Time = duration_cast<milliseconds>(UnitStopTime - ProcessStartTime).count();

    Res = 1;
    MaxCoverage.TotalEdges = edges;
    Printf("[%ld] Slow unit. InsCount: %zd It:%zd\n", Time, edges, TotalNumberOfRuns);
  }
  ...
  auto TimeOfUnit = (UnitStopTime - UnitStartTime).count();
  if (TimeOfUnit > TimeOfLongestUnitInSeconds) {
    auto Time = duration_cast<milliseconds>(UnitStopTime - ProcessStartTime).count();

    Res = 1;
    TimeOfLongestUnitInSeconds = TimeOfUnit;
    // Printf("[%ld] Slow unit: %zd sec:\n", Time, TimeOfLongestUnitInSeconds);
    Printf("[%ld] Slow unit. InsCount: %zd It:%zd\n", Time, MaxCoverage.TotalEdges, TotalNumberOfRuns);
  }
...
void Fuzzer::MutateAndTestOne() {
  ...
  /* FIXME unused for now. will have sorted priorities */
  std::vector<std::pair<float, uint32_t> > MutV;

  memcpy(BaseSha1, II.Sha1, sizeof(BaseSha1));
  assert(CurrentUnitData);

  uint8_t *PreviousUnit = new uint8_t[MaxInputLen + 1];
  ...
  for (int i = 0; i < Options.MutateDepth; i++) {
    ...
    MutInfo = MD.Mutate(CurrentUnitData, Size, Options.MutationStart, MaxMutationLen, &MutV);
    ...
    if (size_t NumFeatures = RunOne(CurrentUnitData, Size)) {
      ...
      Printf("Mut:%d (Score: %d)/ Previous unit written to %s\n",
             mut_idx, MD.GetMutatorScore(mut_idx), Path.c_str());
      CheckExitOnSrcPosOrItem();

      /* Mark successfull Mutator */
      if (Options.DeathMode > 0) {
        MD.IncreaseMutatorScore(mut_idx);
        MD.IncreaseMutatorBucketScore((uint32_t)(std::get<0>(MutInfo) / (MaxMutationLen / (Options.ScoreBuckets - 1))));
      }
    }
    StopTraceRecording();
    int idx = MD.GetLastMutationIdx();
...
void Fuzzer::MinimizeCrashLoop(const Unit &U) {
  ...
  while (!TimedOut() && TotalNumberOfRuns < Options.MaxNumberOfRuns) {
    ...
    for (int i = 0; i < Options.MutateDepth; i++) {
      MutInfo = MD.Mutate(CurrentUnitData, U.size(), -1, MaxMutationLen, &MutV);
      size_t NewSize = std::get<2>(MutInfo);
...
size_t LLVMFuzzerMutate(uint8_t *Data, size_t Size, size_t MaxSize) {
  assert(fuzzer::F);
  return std::get<2>(fuzzer::F->GetMD().DefaultMutate(Data, Size, -1, MaxSize, NULL));
}
}  // extern "C"
